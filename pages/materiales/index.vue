<template>
  <div>
    ...
    <Banner />
    <div>
      <div class="bg-slate-200 w-full  px-5 py-4 space-y-5 sm:py-8 md:py-12 sm:space-y-8 md:space-y-16 max-w-7xl" v-for="material of materiales" :key="materiales.slug">
        <nuxt-link :to="{ name: 'materiales-slug', params: { slug: material.slug } }">
              ...
              <div class="my-4 divide-y-2 divide-gray-700">
                <div class="py-6 flex flex-wrap md:flex-nowrap place-content-center">
                  <div class="mx-8 md:w-64 md:mb-0 mb-6 flex-shrink-0 flex flex-col justify-center text-right border-2 border-gray-400 border-opacity-60 rounded-lg px-5 md:justify-center m:text-center place-content-center">
                    <span class="font-semibold text-xl text-gray-900">Curso: {{material.curso}}</span>
                    <span class="font-semibold text-xl text-gray-900">Area: {{material.area}}</span>
...
<script>
  export default {
      async asyncData({ $content, params }) {
          const materiales = await $content('materiales', params.slug)
